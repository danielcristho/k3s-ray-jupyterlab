resource "null_resource" "cache_image" {
  ...
  provisioner "local-exec" {
    command = "wget -O /tmp/alpine_320.qcow2 ${var.alpine_320_img_url}"
  }
}
resource "libvirt_volume" "base" {
  name       = "base.qcow2"
  source     = "/tmp/alpine_320.qcow2"
  pool       = "default"
  format     = "qcow2"
...
resource "libvirt_cloudinit_disk" "master_commoninit" {
  ...
  user_data = templatefile("${path.module}/config/.cloud_init.tftpl", {
    hostname = var.master_hostname[count.index]
  })
  network_config = templatefile("${path.module}/config/network_config.tftpl", {
    interface = var.interface
    ip_addr   = var.master_ips[count.index]
  })
}

resource "libvirt_volume" "master_alpine_320-qcow2" {
  count          = length(var.master_hostname)
  name           = "alpine_320-${var.master_hostname[count.index]}.qcow2"
...
resource "libvirt_domain" "master" {
  ...
  disk {
    volume_id = element(libvirt_volume.master_alpine_320-qcow2.*.id, count.index)
  }
...
resource "libvirt_volume" "app_alpine_320-qcow2" {
  count          = length(var.app_hostname)
  name           = "alpine_320-${var.app_hostname[count.index]}.qcow2"
  base_volume_id = libvirt_volume.base.id
  pool           = "default"
  size           = var.app_disk
}
...
resource "libvirt_domain" "application_server" {
  ...
  disk {
    volume_id = element(libvirt_volume.app_alpine_320-qcow2.*.id, count.index)
  }
